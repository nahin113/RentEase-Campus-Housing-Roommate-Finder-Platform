...
export interface FlatProperty {
  ...
  neighborhoodLabel?: string
  type: "Private Room" | "Entire Flat" | "Shared Co-Living" | string
  targetAudience: "bachelor" | "family" | "both" | string
  amenities?: string[]
  image: string
  images?: string[]
  status: "available" | "rented" | string
...
export function PropertyPostCard({ flat }: PropertyPostCardProps) {
  ...
  const [isSaved, setIsSaved] = useState(false)
  const [currentImageIdx, setCurrentImageIdx] = useState(0)
  const [user, setUser] = useState<{ name?: string; image?: string; accountType?: string; role?: string }>({})

  useEffect(() => {
    const fetchHostDetails = async () => {
      // 1. Check if user data was ALREADY populated by the backend controller
      const populatedUser = flat.posterUserId?.name ? flat.posterUserId : flat.landlordId?.name ? flat.landlordId : null;
      
      if (populatedUser && typeof populatedUser === 'object') {
        setUser({
          name: populatedUser.name,
          image: populatedUser.image,
          accountType: populatedUser.accountType || populatedUser.role,
        });
        return;
      }

      // 2. Extract string ID fallback (prioritize posterUserId for sublets, landlordId for landlords)
      const targetUserId = typeof flat.posterUserId === 'string' 
        ? flat.posterUserId 
        : typeof flat.landlordId === 'string' 
        ? flat.landlordId 
        : null;
      ...
      try {
        const userData = await publicFetch(`/api/users/${targetUserId}`);
        if (userData?.data) {
          setUser(userData.data);
        }
      } catch (err) {
  ...
  // Dynamically resolve host metadata with appropriate role fallback badges
  const landlord = flat.landlord || {
    name: user?.name || "Verified Host",
    avatar: user?.image || "https://images.unsplash.com/photo-1534528741775-53994a69daeb?w=150&auto=format&fit=crop&q=80",
    badge: user?.accountType || (flat.postedByRole === "renter" ? "Sublet Host" : "Landlord"),
    timestamp: flat.createdAt
      ? new Date(flat.createdAt).toLocaleDateString("en-US", { month: "short", day: "numeric" })
      : "Just now"
  };
    ...
    <article className="w-full bg-white border border-gray-150 rounded-3xl overflow-hidden shadow-sm hover:shadow-md transition-all duration-300 font-sans flex flex-col justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-1.5">
                ...
                {flat.postedByRole === "renter" && (
                  <span className="text-[9px] bg-purple-50 text-purple-700 border border-purple-100 font-bold px-2 py-0.5 rounded-md uppercase tracking-wider scale-90">
                    Sublet
                  </span>
                )}
                {flat.targetAudience === "bachelor" && (
      ...
      <div className="p-4 flex items-center justify-between border-t border-gray-50 bg-gray-50/30">
        <div className="flex items-center gap-1">
          ...
          >
            <MessageSquare className="w-4 h-4" />
            <span className="hidden sm:inline">Ask Host</span>
          </Link>
