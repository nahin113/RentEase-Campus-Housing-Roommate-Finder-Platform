    ...
    <>
      <header className="absolute top-0 left-0 z-50 w-full px-6 py-4 flex items-center justify-between bg-transparent">
        <div className="container mx-auto flex items-center justify-between w-full gap-4">
          ...
          {/* 1. LOGO */}
          <Link href="/" className="flex items-center gap-2 bg-white/80 backdrop-blur-md px-4 py-2 rounded-full border border-gray-100 shadow-sm">
            <Image src="https://i.ibb.co.com/h1nKT2NW/rentease.png" alt="Logo" height={30} width={30} />
            <span className="font-bold text-[#1C1E1B] tracking-tight text-sm">RentEase</span>
          </Link>
              ...
              <Link key={link.href} href={link.href}>
                <Button
                  ...
                  className={`rounded-full text-xs font-semibold px-4 h-8 ${
                    isActive(link.href) ? "bg-[#4E654C] text-[#F4EFEA]" : "text-zinc-700"
                  }`}
                >
                  {link.name}
                </Button>
            ...
            <DropdownMenu open={dropdownOpen} onOpenChange={setDropdownOpen}>
              <DropdownMenuTrigger className="flex items-center gap-2 outline-none focus:outline-none select-none cursor-pointer group">
                <Avatar className="h-7 w-7 border">
                  <AvatarImage src={user?.image || "https://images.unsplash.com/photo-1534528741775-53994a69daeb?w=100"} />
                  <AvatarFallback className="text-[10px] font-bold">RE</AvatarFallback>
                </Avatar>
              ...
              <DropdownMenuContent align="end" className="w-44 p-1.5 rounded-2xl bg-white border border-zinc-200 shadow-xl">
                ...
                {user ? (
                  <>
                    ...
                    <DropdownMenuSeparator />
                    <DropdownMenuItem onClick={handleSignOut} className="px-2.5 py-1.5 text-xs font-semibold text-red-600 rounded-xl">
                  ...
                  <>
                    ...
                    <DropdownMenuItem onClick={() => openAuthModal("signin")} className="px-2.5 py-1.5 text-xs font-semibold rounded-xl cursor-pointer">
                      Sign In
                    </DropdownMenuItem>
                    <DropdownMenuItem onClick={() => openAuthModal("signup")} className="px-2.5 py-1.5 text-xs font-semibold text-[#4E654C] bg-[#4E654C]/5 rounded-xl cursor-pointer">
                      Create Account
                    </DropdownMenuItem>
                  </>
                )}
